...
auth.onAuthStateChanged((user) => {
  ...
  currentUser = user;
  els.who.textContent = user.displayName || user.email || 'User';
...
function normalizeEntryDate(dateValue) {
  ...
  if (dateValue instanceof Date) return formatDateISO(dateValue);
  if (typeof dateValue.toDate === 'function') return formatDateISO(dateValue.toDate());
  return null;
}

function getEntryUid(entryDoc) {
  const pathParts = entryDoc.ref?.path?.split('/') || [];
  return pathParts[1] || entryDoc.ref?.parent?.parent?.id || null;
}

async function calculateGlobalLeaderboard() {
  try {
    const now = new Date();
    const currentWeek = getWeekBounds(now, 'current');
    const previousWeek = getWeekBounds(now, 'previous');

    const [entriesSnapshot, profileSnapshot] = await Promise.all([
      db.collectionGroup('entries').get(),
      db.collection('users').get(),
    ]);

    console.log('Total entries fetched:', entriesSnapshot.docs.length);
    console.log('First entry data:', entriesSnapshot.docs[0]?.data());
    console.log('First entry path:', entriesSnapshot.docs[0]?.ref.path);

    const groupedByUser = {};

    entriesSnapshot.forEach((entryDoc) => {
      const entryData = entryDoc.data() || {};
      const uid = entryDoc.ref?.path?.split('/')[1] || null;
      const entryDate = normalizeEntryDate(entryData.date);

      if (!uid || !entryDate) return;

      const co2Value = Number.isFinite(Number(entryData.co2_kg || 0))
        ? Number(entryData.co2_kg || 0)
        : 0;

      if (entryDate >= currentWeek.startStr && entryDate <= currentWeek.endStr) {
        if (!groupedByUser[uid]) {
          groupedByUser[uid] = { currentTotal: 0, previousTotal: 0 };
        }
        groupedByUser[uid].currentTotal += co2Value;
      }

      if (entryDate >= previousWeek.startStr && entryDate <= previousWeek.endStr) {
        if (!groupedByUser[uid]) {
          groupedByUser[uid] = { currentTotal: 0, previousTotal: 0 };
        }
        groupedByUser[uid].previousTotal += co2Value;
      }
    });

    console.log('Unique users found:', Object.keys(groupedByUser).length);
    console.log('User UIDs:', Object.keys(groupedByUser));

    const userProfiles = {};
    profileSnapshot.docs.forEach((profileDoc) => {
      userProfiles[profileDoc.id] = profileDoc.data() || {};
    });
    console.log('User profiles:', userProfiles);

    const rankedUsers = profileSnapshot.docs.map((profileDoc) => {
      const uid = profileDoc.id;
      const profileData = userProfiles[uid] || {};
      const totals = groupedByUser[uid] || { currentTotal: 0, previousTotal: 0 };
      const currentTotal = totals.currentTotal || 0;
      const previousTotal = totals.previousTotal || 0;
      const buildingBaseline = previousTotal === 0;
      const percentChange = buildingBaseline
        ? null
        : Number((((previousTotal - currentTotal) / previousTotal) * 100).toFixed(1));
      ...
      return {
        userId: uid,
        name: profileData.displayName || profileData.name || profileData.email || fallbackName || 'User',
        currentTotal,
        previousTotal,
        percentChange,
        buildingBaseline,
    ...
    console.error('Leaderboard error code:', error?.code);
...
async function renderLeaderboard() {
  ...
  try {
    const rows = await calculateGlobalLeaderboard();

    if (rows.length <= 1) {
      els.leaderboardBody.innerHTML = `<p class="leaderboard-empty">You're the first one here — invite others to compete!</p>`;
      return;
    }

    const rankedRows = rows.filter((row) => !row.buildingBaseline)
      .sort((a, b) => (b.percentChange ?? -Infinity) - (a.percentChange ?? -Infinity));
    const baselineRows = rows.filter((row) => row.buildingBaseline);

    const rendered = [];
    rankedRows.forEach((row, index) => {
      rendered.push(renderLeaderboardRow(row, index + 1));
    });
...
function renderLeaderboardRow(row, rank) {
  ...
  let changeLabel = '';
  let changeClass = 'leaderboard-change';
  let secondaryText = `(${row.currentTotal.toFixed(1)} kg CO₂e this week)`;
  ...
  const rowClass = row.isCurrentUser ? 'leaderboard-row current-user' : 'leaderboard-row';
...
function getWeekBounds(referenceDate, mode) {
  ...
  return {
    ...
    startStr: formatDateISO(previousStart),
    endStr: formatDateISO(previousEnd),
  };
}
